...
import { fetchCroxPoolUserDataAsync, fetchDualFarmUserDataAsync, fetchFarmUserDataAsync } from "state/actions";
...
import { usePriceCakeBusd, useCroxPoolSymbol, useDualFarms, useFarms, useCroxPools, useCroxmasPools } from "../../../state/hooks";
...
const InvestorCard = () => {
  ...
  const poolList = useCroxPools()
  const stakedPoolList = []
  for (let j = 0; j < poolList.length; j++) {
    if (poolList[j].userData && getBalanceNumber(new BigNumber(poolList[j].userData.stakedBalance)) !== 0) {
      stakedPoolList.push(poolList[j])
    }
  }
  ...
  let croxmasStaked = new BigNumber(0)
  for (let i = 0; i < croxmasPools.length; i++) {
    if (croxmasPools[i].userData) {
      croxmasStaked = new BigNumber(croxmasStaked).plus(new BigNumber(croxmasPools[i].userData.stakedBalance))
    }
  }
  const totalstaked = getBalanceNumber(new BigNumber(stakedBalance).plus(new BigNumber(nextgenstaked)).plus(new BigNumber(croxmasStaked)))
  ...
  return (
    <>
                  ...
                  <tr>
                    <td>
                      <Flex alignItems='center' ml='15px'>
                        <FaWallet style={{ fontSize: '14px', color: 'white' }} />
      ...
      <ReactModal isOpen={isViewPortfolioModalOpen} onRequestClose={() => closeModal()} style={customStyles}>
        <ViewPortfolioModal onDismiss={() => closeModal()} stakedFarmList={stakedFarmList} stakedDualFarmList={stakedDualFarmList} stakedPoolList={stakedPoolList} stakedNextGenList={stakedNextGenList} />
      </ReactModal>
    </>
  );
};
