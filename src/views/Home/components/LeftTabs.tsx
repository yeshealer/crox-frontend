...
import { Text, Flex, Input, useMatchBreakpoints, } from 'crox-new-uikit'
...
import Whitelist from 'constants/pancakeswap.json'
...
const LeftTabs: React.FC<LeftTabsProps> = ({
  ...
  for (let i = 0; i < stakedFarmList.length; i++) {
    ...
    farmLpWorth[i] = new BigNumber(farmApy[i].totalValue)
      ...
      .toFixed(2);
    if (stakedFarmList[i].userData.stakedBalance) {
      totalStakedAmount += getBalanceNumber(stakedFarmList[i].userData.stakedBalance) * farmLpWorth[i]
    }
  }
  ...
  for (let i = 0; i < stakedDualFarmList.length; i++) {
    ...
    dualFarmLpWorth[i] = new BigNumber(dualFarmApyGroup[i].totalValue)
      ...
      .toFixed(2);
    if (stakedDualFarmList[i].userData.stakedBalance) {
  ...
  for (let i = 0; i < stakedPoolList.length; i++) {
    ...
    if (stakedPoolList[i].userData.stakedBalance) {
      totalStakedAmount += getBalanceNumber(stakedPoolList[i].userData.stakedBalance) * poolLpWorth[i]
    }
  }
  ...
  for (let i = 0; i < stakedNextGenList.length; i++) {
    ...
    nextgenLpWorth[i] = stakedNextGenList[i].tokenPrice
    if (stakedNextGenList[i].userData.stakedBalance) {
  ...
  const loadedData = JSON.stringify(Whitelist)
  const tokenData = JSON.parse(loadedData)
      ...
      {!isMd && !isSm && !isXs && !isLg ? (
        <Box sx={{ flexGrow: 1, bgcolor: '#171923', display: 'flex', height: 560 }}>
          ...
          >
            ...
            {/* <Tab icon={<Icon icon="akar-icons:arrow-right-left" style={{ fontSize: '24px' }} />} iconPosition='start' label="TRANSACTIONS" style={tabstyle} {...a11yProps(1)} className="leftTab" /> */}
                ...
                <CardActionArea>
                  <CardContent className="cardContent">
                    <Typography variant="h5" component="div">
                      ...
                      <Text style={{ textAlign: 'center' }} fontSize="18px">${netWalletAmount.toFixed(2)}</Text>
                    </Typography>
                  </CardContent>
                </CardActionArea>
              </Card>
                ...
                <CardActionArea>
                  <CardContent className="cardContent">
                    <Typography variant="h5" component="div">
                      ...
                      <Text style={{ textAlign: 'center' }} color='textSubtle' fontSize="18px">${totalStakedAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
                    </Typography>
                  </CardContent>
                </CardActionArea>
              </Card>
          ...
          <TabPanel value={value} index={0}>
            ...
            {/* <div className='headerCard'>
              <Card sx={{ maxWidth: 250 }} className='cardbody' >
                <CardActionArea>
                  <CardContent className="cardContent">
                    <Typography variant="h5" component="div" className='typography_text'>
                      <Text fontSize="20px" mr='3px' color='white'>Net Worth:</Text>
                      <Text fontSize="20px" color='white'>${divider(netWalletAmount + totalStakedAmount)}</Text>
                    </Typography>
                  </CardContent>
                </CardActionArea>
              </Card>
            </div> */}
            <table className="table tablesorter " id="plain-table" style={{ marginTop: 60 }}>
              ...
              <tbody className="tbody_wallet">
                <tr>
                  ...
                  <td>
                    ${eggPrice.toLocaleString(undefined, { minimumFractionDigits: 3, maximumFractionDigits: 3 })}
                  </td>
                  <td>
              ...
              <tbody className="tbody_transaction">
                <tr>
                  ...
                  <td style={{ display: 'flex', justifyContent: 'center' }}>
                    <Button href={`https://bscscan.com/address/${account}`} target="_blank">
          ...
          <TabPanel value={value} index={2}>
            ...
            <div className='headerCard headerCard_yield'>
              <Flex alignItems="center" m='10px 25px'>
              ...
              {/* <Typography variant="h5" component="div" className='typography_text'>
                      ...
                      <Text fontSize="20px" color='white'>{divider(totalValue.toNumber())}</Text>
                    </Typography> */}
              {/* </CardContent>
                </CardActionArea>
              </Card> */}
            ...
            <table className="table tablesorter" id="plain-table">
              ...
              <tbody className="tbody_yield">
                {stakedFarmList.map((list, index) => (
                  <StakedFarmList
                    farmPid={list.pid}
                    farmImage={farmImage[index]}
                    lpSymbol={list.lpSymbol}
                    farmApy={farmApy[index].apy}
                    stakedBalance={list.userData.stakedBalance}
                    lpWorth={farmLpWorth[index]}
                ...
                {stakedPoolList.map((list, index) => (
                  <StakedPoolList
                    farmPid={list.pid}
                    poolImage={poolImage[index]}
                    lpSymbol={list.lpSymbol}
                    poolApy={poolApy[index].apy}
                    stakedBalance={list.userData.stakedBalance}
                    lpWorth={poolLpWorth[index]}
                    earnings={list.userData.earnings}
                    multiplier={list.multiplier}
                  />
                ))}
        ...
        >
          ...
          <Flex justifyContent="center">
            ...
            >
              ...
              {/* <Tab icon={<Icon icon="akar-icons:arrow-right-left" style={{ fontSize: '24px' }} />} iconPosition='start' label="TRASACTIONS" style={tabstyle} className="TabIcon leftTab" {...a11yProps(1)} /> */}
