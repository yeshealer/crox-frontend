...
const WithdrawModal: React.FC<WithdrawModalProps> = ({ onDismiss, stakedBalance, stakedAmountStr }) => {
    const { account } = useWallet();
    const [inputValue, setInputValue] = useState(null)
    const [isWaiting, setIsWaiting] = useState(false)
    const [isFail, setIsFail] = useState(false)
    const [isSuccess, setIsSuccess] = useState(false)
    const [headerText, setHeaderText] = useState("Withdraw CROX")
    ...
    useEffect(() => {
        if (isWaiting) {
            setHeaderText("Withdrawing CROX")
        } else if (isFail) {
            ...
            <div>
                {isWaiting ? (
                    <Flex flexDirection='column' alignItems="center">
                        ...
                        />
                        <Text fontSize='18px'>Withdrawing {inputValue}CROX</Text>
                    </Flex>
                ) : (isFail ? (
                    <Flex flexDirection='column' alignItems="center">
                        <Player
                            autoplay
                            loop
                            src="https://assets9.lottiefiles.com/packages/lf20_ed9D2z.json"
                    ...
                    <Flex flexDirection='column' justifyContent='flex-end' alignItems='end'>
                        <Text fontSize='14px'>{stakedBalance} CROX Available</Text>
