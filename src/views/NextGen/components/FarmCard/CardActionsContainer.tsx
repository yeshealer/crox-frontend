    ...
    <>
      ...
      <Action>
        ...
        <StakeContainer style={removed ? { width: '45%'} : {width: '25%'}}>
          ...
          {/*   {!removed && !farm.isDualFarm && farm.tokenSymbol !== 'PUNKS' && (
         <Text fontSize="12px" color="textSubtle" style={{ marginTop: "1%" }}>
              {(farm as any).maxStakeLimit && !farm.isDualFarm ? `Max Stake Limit: ${getBalanceNumber((farm as any).maxStakeLimit)} ${lpName} per wallet` : "Max Stake: 8000 CROX per wallet"}
            </Text>
          )} */}
        </StakeContainer>
        {removed ? (<div />) : (
