    ...
    <Flex justifyContent="space-between" alignItems="center">
        ...
        {isDualFarm || isWhalePool || isShrimpPool ? (
          <>
            <Flex>
              ...
              <Text bold fontSize="15px" color="textSubtle">Earned</Text>
            </Flex>
            <Text bold fontSize="18px" color="textSubtle">{displayBalance1}</Text>
            <Flex>
              <Text bold fontSize="15px" color="#2d74c4" mr='3px'>{secondSymbol}</Text>
              <Text bold fontSize="15px" color="textSubtle">Earned</Text>
            </Flex>
            <Text bold fontSize="18px" color="textSubtle">{displayBalance2}</Text>
          </>
        ) : (
          <>
            <Flex mt={removed || ismobile ? '5px' : '20px'}>
              <Text bold fontSize="15px" color="#2d74c4" mr='3px'>{secondSymbol}</Text>
              <Text bold fontSize="15px" color="textSubtle">Earned</Text>
            </Flex>
            <Text bold fontSize="18px" color="textSubtle">{displayBalance1}</Text>
      ...
      <BalanceAndCompound>
        {((rawEarningsBalance1 === 0 && rawEarningsBalance2 === 0) || pendingTx || today < nextHarvestUntil) ? (
          <Button disabled style={{borderRadius: '5px', height: '32px'}} mt={!isDualFarm && !isWhalePool && !isShrimpPool && !removed ? "20px" : "0"}>Claim Rewards</Button>
        ) : (
          <Button
            onClick={async () => {
              setPendingTx(true);
