...
import { useFarms, useDualFarms, usePriceBnbBusd, usePriceCakeBusd } from "state/hooks";
import useRefresh from "hooks/useRefresh";
import { fetchFarmUserDataAsync } from "state/actions";
import useI18n from "hooks/useI18n";
import { getAPYAndTVLOfFarm, getAPYAndTVLOfNGPool } from "utils/defi";
import Select, { OptionProps } from "components/Select/Select";
import FarmCard, { FarmWithStakedValue } from "./components/FarmCard/FarmCard";
import DualFarmCard, { DualFarmWithStakedValue } from "./components/DualFarmCard/DualFarmCard";
...
const Farms: React.FC<FarmsProps> = (farmsProps) => {
  ...
  const dualFarmsLP = useDualFarms(true);
  console.log("dualfarmsLP => ", dualFarmsLP);
  ...
  const activeDualFarms = dualFarmsLP.filter((it) => (it as any).active);
  const inactiveDualFarms = dualFarmsLP.filter((it) => !(it as any).active);
  ...
  const stakedOnlyDualFarms = activeDualFarms.filter(
    (farm) =>
      farm.userData &&
      new BigNumber(farm.userData.stakedBalance).isGreaterThan(0)
  );
  ...
  const stakedInactiveDualFarms = inactiveDualFarms.filter(
    (farm) =>
      farm.userData &&
      new BigNumber(farm.userData.stakedBalance).isGreaterThan(0)
  );
  ...
  const dualFarmsList = useCallback(
    (farmsToDisplay, removed?: boolean) => {
      // const cakePriceVsBNB = new BigNumber(farmsLP.find((farm) => farm.pid === CAKE_POOL_PID)?.tokenPriceVsQuote || 0)
      let farmsToDisplayWithAPY: DualFarmWithStakedValue[] = farmsToDisplay.map(
        (farm) => {
  ...
  const dualFarmsStakedMemoized = useMemo(() => {
    let farmsStaked = [];

    const sortFarms = (farms: DualFarmWithStakedValue[]): DualFarmWithStakedValue[] => {
      switch (sortOption) {
        case "apy":
          return orderBy(
            farms,
            (farm: DualFarmWithStakedValue) => farm.apy1,
            "desc"
          );
        case "earned":
          return orderBy(
        ...
        // case 'liquidity':
        //   return orderBy(farms, (farm: DualFarmWithStakedValue) => Number(farm.liquidity), 'desc')
        default:
          return farms;
      }
    };
    ...
    if (isActive) {
      farmsStaked = dualFarmsList(
        stakedOnly ? stakedOnlyDualFarms : activeDualFarms,
        false
      );
    }
    if (isInactive) {
      farmsStaked = dualFarmsList(
        stakedOnly ? stakedInactiveDualFarms : inactiveDualFarms,
        true
      );
    }
    ...
    isActive,
    isInactive,
    stakedInactiveDualFarms,
    stakedOnly,
    stakedOnlyDualFarms,
  ]);

  console.log("farmsStakedMemoized => ", farmsStakedMemoized)
  console.log("dualFarmsStakedMemoized => ", dualFarmsStakedMemoized)
      ...
      <div>
        ...
        <FlexLayout>
          <Route exact path={`${path}`}>
            {dualFarmsStakedMemoized.map((farm) => (
              <DualFarmCard
                key={farm.pid}
                farm={farm}
                account={account}
