...
const FarmCard: React.FC<FarmCardProps> = ({
  ...
  //   });
  const farmAPYBal = new BigNumber(0);
  const farmAPY =
    farmAPYBal.times(new BigNumber(100)).toNumber().toLocaleString(undefined, {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    });
      ...
      <FCard onClick={() => !clickApy && setShowExpandableSection(!showExpandableSection)}>
        {removed ? (
          ...
          <HeaderContainer>
            ...
            <CakeLP>
              <Flex flexDirection="column" alignItems="flex-start">
                <Flex justifyContent="center">
                  <Text color="#2d74c4" bold style={{ border: '2px solid #2d74c4', borderRadius: '20px', padding: '0 5px' }}>CAKE LP</Text>
                </Flex>
              </Flex>
            </CakeLP>
            <LpWorth>
              <Text color="#2d74c4" fontSize="15px">Lp Worth</Text>
              <Text color="textSubtle" fontSize="15px">${lpWorth}</Text>
            </LpWorth>
            <EarnToken>
              <Flex justifyContent='center'>
                <Text color="#2d74c4" fontSize="15px" mr='3px'>{earnLabel}</Text>
                <Text color="textSubtle" fontSize="15px">Earned</Text>
              </Flex>
              <Text color="textSubtle">{displayBalance}</Text>
            </EarnToken>
            <Apr>
              <Text color="textSubtle" style={{ fontSize: "15px" }}>{TranslateString(352, "APR")}</Text>
              <Flex justifyContent='center'>
                <Text color="textSubtle" bold style={{ display: "flex", alignItems: "center", fontSize: "15px" }}>
                  {farm.apy ? (
                    <>
                      {farmAPY}%
                    </>
            ...
            <TotalValue>
              <Text color="textSubtle" fontSize="15px">Liquidity</Text>
              <Text color="textSubtle" fontSize="15px">{totalValueFormated}</Text>
            </TotalValue>
            <MultiplierTag>
              <Text color="textSubtle" fontSize="15px">Multiplier</Text>
              <Text color="textSubtle" fontSize="15px">{farm.multiplier}</Text>
            </MultiplierTag>
            <Fee>
              <Text color="textSubtle" fontSize="15px">Fees</Text>
              <Text color="textSubtle" bold fontSize="15px">{farm.depositFeeBP / 100}%</Text>
            </Fee>
            <Harvest>
              <Text color="textSubtle" fontSize="15px">{TranslateString(10006, "Harvest Lock")}</Text>
              <Text color="textSubtle" bold fontSize="15px">{farm.harvestInterval / 3600.0} hours</Text>
            </Harvest>
            <ExpandableSectionButton
              onClick={() => setShowExpandableSection(!showExpandableSection)}
              expanded={showExpandableSection}
