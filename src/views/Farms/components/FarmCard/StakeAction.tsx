...
const StakeAction: React.FC<FarmCardActionsProps> = ({
  ...
  const renderStakingButtons = () => {
    return rawStakedBalance === 0 ? (
      ...
      <IconButtonWrapper>
        <Button size="sm" style={{ borderRadius: "5px", backgroundColor: "#483f5a", height: '32px', fontSize: '16px' }}
          onClick={
            async () => {
              setPendingTx(true);
              await onUnstake();
  ...
  return (
    <Flex justifyContent='space-between' alignItems='center'>
      <Flex flexDirection='column'>
        ...
        <Text fontSize='18px' color='textSubtle' bold>${yourLPValue.toFixed(3)}</Text>
      </Flex>
      <BtnGrp>
