  ...
  const renderStakingButtons = () => {
    return rawStakedBalance === 0 ? (
      <>
        ...
        <Button style={{ height: "32px", borderRadius: "5px", fontSize: '16px' }} fullWidth onClick={onPresentDeposit}>{TranslateString(999, 'Stake')}</Button>
      </>
    ) : (
      <>
        {minFirstDeposit && (
  ...
  return (
    <Flex justifyContent="space-between" alignItems="space-between" flexDirection='column'>
      <Flex style={{ width: 'max-content' }} justifyContent='center' mt='3px'>
        ...
        <Text textTransform="uppercase" fontSize="15px" color="textSubtle" bold>
      ...
      <Flex justifyContent='space-between' mt='5px'>
        <Flex flexDirection='column'>
          ...
          {isDualFarm && (<Text style={{ fontWeight: "initial" }} fontSize="18px" color="textSubtle" bold>${yourLPValue.toFixed(3)}</Text>)}
        </Flex>
        <BtnGrp>
