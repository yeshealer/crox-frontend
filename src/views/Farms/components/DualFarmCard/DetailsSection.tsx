        ...
        <DepositContainer>
          {!removed && (
            <Flex justifyContent="space-between">
              <Text fontSize="15px" color="textSubtle" bold>{TranslateString(23, "Rewards End Block")}:</Text>
              <LinkExternal href={`https://bscscan.com/block/countdown/${bonusEndBlock}`}>{bonusEndBlock}</LinkExternal>
            </Flex>
          )}
          {!removed && (
            <Flex justifyContent="space-between">
              <Text fontSize="15px" color="textSubtle" bold>Total Rewards</Text>
              <Flex flexDirection="column">
                {reward1 && <Text fontSize="15px" color="textSubtle" bold style={{textAlign: "right"}}>{reward1} CROX</Text>}
                {reward2 && (<Text fontSize="15px" color="textSubtle" bold style={{textAlign: "right"}}>{`${reward2} ${tokenSymbol}`} </Text>)}
              </Flex>
            </Flex>
          )}
          {!removed && (isDualFarm || isLPToken) && (
            <Flex justifyContent="space-between">
              <Text fontSize="15px" color="textSubtle" bold>Withdrawable LP:</Text>
              <Text fontSize="15px" color="textSubtle" bold>{redeemable.toFixed(3)}</Text>
            </Flex>
          )}
        </DepositContainer>
      ) : (
      ...
      {isDualFarm || isLPToken ? (
        <DetailContainer>
          ...
          <Flex justifyContent="flex-start">
            <Link external href={bscScanAddress} bold fontSize="18px">
