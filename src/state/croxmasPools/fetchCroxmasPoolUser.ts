...
export const fetchCroxmasPoolsUserAllowances = async (account: string) => {
...
export const fetchCroxmasPoolsUserRedeemable = async (account: string) => {
...
export const fetchCroxmasPoolsUserNextHarvestUntil = async (account: string) => {
  const calls = farmsConfig
    .filter((it) => !it.active)
    .map((farm) => {
      return {
...
export const fetchCroxmasPoolsUserTokenBalances = async (account: string) => {
  const calls = farmsConfig.map((farm) => {
    const lpContractAddress = farm.isTokenOnly
      ? farm.tokenAddresses[CHAIN_ID]
      : farm.lpAddresses[CHAIN_ID];
...
export const fetchCroxmasPoolsUserStakedBalances = async (account: string) => {
  const calls = farmsConfig
    .filter((it) => !it.active)
    .map((farm) => {
      return {
...
export const fetchCroxmasPoolsUserEarnings = async (account: string) => {
  const calls = farmsConfig
    .filter((it) => !it.active)
    .map((farm) => {
      return {
