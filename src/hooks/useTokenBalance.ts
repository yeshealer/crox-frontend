...
import croxmaspools from "config/constants/croxmaspools"
import grandpools from "config/constants/grandpool";
  ...
  useEffect(() => {
    async function fetchTotalSupply() {
      ...
      for (let i = 0; i < croxmaspools.length; i++) {
        func.push(
          cakeContract.methods
            .balanceOf(croxmaspools[i].poolAddress[process.env.REACT_APP_CHAIN_ID])
            .call()
        );
      }
      for (let i = 0; i < grandpools.length; i++) {
        func.push(
          cakeContract.methods
            .balanceOf(grandpools[i].poolAddress[process.env.REACT_APP_CHAIN_ID])
            .call()
        );
      }
